...
app.get("/", (req, res) => {
    res.send(
        "API is running. Use POST /generate-pdf to generate a PDF or GET /print-badge to print a badge."
    );
});
...
app.post("/generate-pdf", async (req, res) => {
    try {
        ...
        const footerResponse = await axios.get(footerUrl, {
            responseType: "arraybuffer",
        });
        ...
        const pageHeight =
            type === "ind" ? 595 : 300 + memberQRCodes.length * 150; // Chiều cao động cho nhóm, khổ A5 cho cá nhân
        ...
        doc.registerFont(
            "Poppins-Medium",
            path.join(__dirname, "fonts/Poppins-Medium.ttf")
        );
        doc.registerFont(
            "Poppins-SemiBold",
            path.join(__dirname, "fonts/Poppins-SemiBold.ttf")
        );
        ...
        if (type === "ind") {
            ...
            doc.font("Poppins-Medium")
                .fontSize(14)
                .text("YOUR GROUP'S BADGES INFORMATION:", { align: "center" });
            doc.moveDown(0.5);
            ...
            memberQRCodes.forEach((member, index) => {
                ...
                doc.font("Poppins-Medium")
                    .fontSize(9)
                    .text(
                        `Use this code for individual check-in.`,
                        60,
                        doc.y + 10,
                        {
                            align: "center",
                ...
                doc.font("Poppins-Medium")
                    .fontSize(18)
                    .text(`#${index + 1}: ${member.name}`, 60, doc.y + 15, {
                        align: "center",
